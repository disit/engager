/*******************************************************************************
 * Engager
 *    Copyright (C) 2016-2018 DISIT Lab http://www.disit.org - University of Florence
 *    This program is free software: you can redistribute it and/or modify
 *    it under the terms of the GNU Affero General Public License as
 *    published by the Free Software Foundation, either version 3 of the
 *    License, or (at your option) any later version.
 *    This program is distributed in the hope that it will be useful,
 *    but WITHOUT ANY WARRANTY; without even the implied warranty of
 *    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *    GNU Affero General Public License for more details.
 *    You should have received a copy of the GNU Affero General Public License
 *    along with this program.  If not, see <http://www.gnu.org/licenses/>.
 ******************************************************************************/
package edu.unifi.disit.userprofiler.service;

import java.util.Date;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import edu.unifi.disit.commons.datamodel.userprofiler.Location;
import edu.unifi.disit.userprofiler.datamodel.LocationDAO;

@Component
public class DeviceLocationServiceImpl implements IDeviceLocationService {

	@Autowired
	LocationDAO locationRepo;

	@Override
	public Location getCurrentLocations(String deviceId, String lang) {
		List<Location> locations = locationRepo.findByDeviceDeviceIdOrderByTimeDesc(deviceId);

		if ((locations != null) && (locations.size() > 0))
			return locations.get(0);

		return null;

	}

	@Override
	public List<Location> getLocations(String deviceId, String lang, Integer page, Integer size, Date from, Date to) {

		// TODO mandage pagination and constrains
		return locationRepo.findByDeviceDeviceId(deviceId);
	}

	@Override
	public Integer countLogs(String deviceId) {
		return locationRepo.countByDeviceDeviceId(deviceId);
	}
}
