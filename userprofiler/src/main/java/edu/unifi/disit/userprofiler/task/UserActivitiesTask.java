...
@EnableScheduling
@Component
public class UserActivitiesTask implements SchedulingConfigurer {

	private static final Logger logger = LoggerFactory.getLogger(UserActivitiesTask.class);

	@Value("${useractivities.task.refresh.cron}")
	private String cronrefresh;

	@Value("${useractivities.task.refresh.duration.minutes}")
	...
	public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
		taskRegistrar.addTriggerTask(new Runnable() {
			...
			public Date nextExecutionTime(TriggerContext triggerContext) {
				...
				logger.debug("next execution for USER ACTIVITIES will be fired at:{}", nextExec);
	...
	private void myTask() {
		...
		logger.debug("updating useractivities");
		...
		for (Device d : devices) {
			...
			logger.debug("updating useractivities, device {}", d.getDeviceId());

			d.setUserActivities(deviceUAService.refreshUserActivies(d.getDeviceId()));
